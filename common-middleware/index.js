...
exports.requireConnectCrmCard = async (req, res, next) => {
  try {
    ...
    return res
      .status(400)
      .json({
        from: "(common-middelware/index/requireConnect) function Error :- ",
...
exports.requiredAuth = async (req, res, next) => {
  try {
    if (req.query.portalId) {
      ...
      req.user = user;
      next();
    } else if(req.params.sumoquoteWebhookId){
        const user = await User.findOne({ sumoquoteWebhookId: req.params.sumoquoteWebhookId });
        if (!user || !user.hubspotPortalId || !user.hubspotRefreshToken) {
          console.log("Hubspot Not Connected");
          return res.redirect("/hubspot/connect");
        }
    ...
    return res
      .status(400)
      .json({
        from: "(common-middelware/index/requiredAuth) function Error :- ",
        message: error.message
      });
  }
};
